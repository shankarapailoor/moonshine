...
func (r *returnCache) Cache(SyzType prog.Type, StraceType straceTypes.Type, arg prog.Arg) {
	resDesc := ResourceDescription{
		Type: straceTypes.GetSyzType(SyzType),
		Val:  StraceType.String(),
	}
	(*r)[resDesc] = arg
}

func (r *returnCache) Get(SyzType prog.Type, StraceType straceTypes.Type) prog.Arg {
	resDesc := ResourceDescription{
		Type: straceTypes.GetSyzType(SyzType),
		Val:  StraceType.String(),
	}
...
type Context struct {
	Cache             returnCache
	Prog              *prog.Prog
	CurrentStraceCall *straceTypes.Syscall
	CurrentSyzCall    *prog.Call
	CurrentStraceArg  straceTypes.Type
	State             *tracker.State
	Target            *prog.Target
...
func ParseProg(trace *straceTypes.Trace, target *prog.Target) (*Context, error) {
	...
	for _, s_call := range trace.Calls {
		...
		if _, ok := straceTypes.Unsupported[s_call.CallName]; ok {
			continue
		}
		if s_call.Paused {
...
func parseCall(ctx *Context) (*prog.Call, error) {
	...
	retCall.Ret = straceTypes.ReturnArg(ctx.CurrentSyzCall.Meta.Ret)

	if call := ParseMemoryCall(ctx); call != nil {
		return call, nil
	}
	for i := range retCall.Meta.Args {
		var strArg straceTypes.Type = nil
...
func parseResult(syzType prog.Type, straceRet int64, ctx *Context) {
	if straceRet > 0 {
		...
		straceExpr := straceTypes.NewExpression(straceTypes.NewIntsType([]int64{straceRet}))
		switch syzType.(type) {
...
func parseArgs(syzType prog.Type, straceArg straceTypes.Type, ctx *Context) (prog.Arg, error) {
	if straceArg == nil {
...
func Parse_VmaType(syzType *prog.VmaType, straceType straceTypes.Type, ctx *Context) (prog.Arg, error) {
	npages := uint64(1)
	...
	arg := straceTypes.PointerArg(syzType, 0, npages, nil)
	ctx.State.Tracker.AddAllocation(ctx.CurrentSyzCall, pageSize, arg)
	return arg, nil
}

func Parse_ArrayType(syzType *prog.ArrayType, straceType straceTypes.Type, ctx *Context) (prog.Arg, error) {
	...
	case *straceTypes.Field:
		return Parse_ArrayType(syzType, a.Val, ctx)
	case *straceTypes.PointerType, *straceTypes.Expression, *straceTypes.BufferType:
		return GenDefaultArg(syzType, ctx), nil
	default:
		Failf("Error parsing Array: %s with Wrong Type: %s\n", syzType.FldName, straceType.Name())
	}
	return straceTypes.GroupArg(syzType, args), nil
}

func Parse_StructType(syzType *prog.StructType, straceType straceTypes.Type, ctx *Context) (prog.Arg, error) {
	straceType = PreprocessStruct(syzType, straceType, ctx)
	args := make([]prog.Arg, 0)
	switch a := straceType.(type) {
	case *straceTypes.StructType:
...
func evalFields(syzFields []prog.Type, straceFields []straceTypes.Type, ctx *Context) []prog.Arg {
	args := make([]prog.Arg, 0)
	j := 0
...
func Parse_UnionType(syzType *prog.UnionType, straceType straceTypes.Type, ctx *Context) (prog.Arg, error) {
	...
	default:
		...
		if innerArg, err := parseArgs(innerType, straceType, ctx); err == nil {
			return straceTypes.UnionArg(syzType, innerArg), nil
		} else {
...
func IdentifySockaddrStorageUnion(ctx *Context) int {
	...
	var straceArg straceTypes.Type
...
func Parse_BufferType(syzType *prog.BufferType, straceType straceTypes.Type, ctx *Context) (prog.Arg, error) {
	if syzType.Dir() == prog.DirOut {
		if !syzType.Varlen() {
	...
	if !syzType.Varlen() {
		bufVal = straceTypes.GenBuff(bufVal, syzType.Size())
	...
	return straceTypes.DataArg(syzType, bufVal), nil
}

func Parse_PtrType(syzType *prog.PtrType, straceType straceTypes.Type, ctx *Context) (prog.Arg, error) {
	switch a := straceType.(type) {
	case *straceTypes.PointerType:
		if a.IsNull() {
...
func Parse_ConstType(syzType prog.Type, straceType straceTypes.Type, ctx *Context) (prog.Arg, error) {
	if syzType.Dir() == prog.DirOut {
		return ctx.Target.DefaultArg(syzType), nil
	}
	switch a := straceType.(type) {
	case *straceTypes.Expression:
		...
		return straceTypes.ConstArg(syzType, a.Eval(ctx.Target)), nil
	case *straceTypes.DynamicType:
		return straceTypes.ConstArg(syzType, a.BeforeCall.Eval(ctx.Target)), nil
	case *straceTypes.ArrayType:
		/*
	...
	case *straceTypes.PointerType:
		...
		return straceTypes.ConstArg(syzType, a.Address), nil
	default:
...
func Parse_ResourceType(syzType *prog.ResourceType, straceType straceTypes.Type, ctx *Context) (prog.Arg, error) {
	if syzType.Dir() == prog.DirOut {
		res := straceTypes.ResultArg(syzType, nil, syzType.Default())
	...
	case *straceTypes.Expression:
		...
		if arg := ctx.Cache.Get(syzType, straceType); arg != nil {
			res := straceTypes.ResultArg(arg.Type(), arg.(*prog.ResultArg), arg.Type().Default())
			return res, nil
		}
		res := straceTypes.ResultArg(syzType, nil, val)
		return res, nil
	case *straceTypes.Field:
...
func Parse_ProcType(syzType *prog.ProcType, straceType straceTypes.Type, ctx *Context) (prog.Arg, error) {
	if syzType.Dir() == prog.DirOut {
		return GenDefaultArg(syzType, ctx), nil
	}
	switch a := straceType.(type) {
	case *straceTypes.Expression:
		...
		if val >= syzType.ValuesPerProc {
			return straceTypes.ConstArg(syzType, syzType.ValuesPerProc-1), nil
		} else {
			return straceTypes.ConstArg(syzType, val), nil
		}
	case *straceTypes.Field:
		return parseArgs(syzType, a.Val, ctx)
	case *straceTypes.Call:
...
func GenDefaultArg(syzType prog.Type, ctx *Context) prog.Arg {
	...
	case *prog.StructType:
		...
		return straceTypes.GroupArg(a, inner)
	case *prog.UnionType:
		optType := a.Fields[0]
		return straceTypes.UnionArg(a, GenDefaultArg(optType, ctx))
	case *prog.ArrayType:
...
func serialize(syzType prog.Type, buf []byte, ctx *Context) (prog.Arg, error) {
	...
	case *prog.IntType, *prog.ConstType, *prog.FlagsType, *prog.LenType, *prog.CsumType:
		return straceTypes.ConstArg(a, bufToUint(buf[:syzType.Size()])), nil
	case *prog.ProcType:
		return GenDefaultArg(syzType, ctx), nil
	case *prog.PtrType:
...
func addr(ctx *Context, syzType prog.Type, size uint64, data prog.Arg) (prog.Arg, error) {
	arg := straceTypes.PointerArg(syzType, uint64(0), 0, data)
	ctx.State.Tracker.AddAllocation(ctx.CurrentSyzCall, size, arg)
	return arg, nil
}

func reorderStructFields(syzType *prog.StructType, straceType *straceTypes.StructType, ctx *Context) {
	/*
	...
	*/
	...
	case "bpf_insn_generic", "bpf_insn_exit", "bpf_insn_alu", "bpf_insn_jmp", "bpf_insn_ldst":
		...
		newFields := make([]straceTypes.Type, len(straceType.Fields)-1)
		newFields[0] = straceType.Fields[0]
		newFields[1] = straceTypes.NewExpression(straceTypes.NewIntType(int64(reg)))
...
func GenDefaultStraceType(syzType prog.Type) straceTypes.Type {
	switch a := syzType.(type) {
	case *prog.StructType:
		straceFields := make([]straceTypes.Type, len(a.Fields))
		for i := 0; i < len(straceFields); i++ {
			straceFields[i] = GenDefaultStraceType(a.Fields[i])
		}
		return straceTypes.NewStructType(straceFields)
	case *prog.ArrayType:
		straceFields := make([]straceTypes.Type, 1)
		straceFields[0] = GenDefaultStraceType(a.Type)
		return straceTypes.NewArrayType(straceFields)
	case *prog.ConstType, *prog.ProcType, *prog.LenType, *prog.FlagsType, *prog.IntType:
		return straceTypes.NewExpression(straceTypes.NewIntType(0))
	case *prog.PtrType:
		return straceTypes.NewPointerType(0, GenDefaultStraceType(a.Type))
	case *prog.UnionType:
